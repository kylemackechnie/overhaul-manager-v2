...
export function NrgTcePanel() {
  ...
  async function doExportTce() {
    ...
    try {
      await exportTceAll(activeProject.id, activeProject.name || 'project', lines, orderedWeeks)
    } catch (e) {
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px', flexWrap: 'wrap', gap: '8px' }}>
        ...
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
          ...
          <button className="btn btn-sm" onClick={openTceExport} disabled={exportingTce}>
            {exportingTce ? <span className="spinner" style={{ width: '14px', height: '14px' }} /> : '📊'} Export TCE
          </button>
