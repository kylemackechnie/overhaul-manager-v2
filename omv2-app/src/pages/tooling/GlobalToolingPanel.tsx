...
export function GlobalToolingPanel() {
  ...
  async function load() {
    setLoading(true)
    const [tvData, deptData] = await Promise.all([
      supabase.from('global_tvs').select('*,department:global_departments(id,name)').order('tv_no'),
  ...
  const filtered = tvs.filter(tv=>!search||tv.tv_no.includes(search)||(tv.header_name||'').toLowerCase().includes(search.toLowerCase()))
    ...
    <div style={{padding:'24px',maxWidth:'1000px'}}>
      ...
      : filtered.length===0 ? (
        ...
        <div className="card" style={{padding:0,overflow:'hidden'}}>
          <table>
            <thead><tr><th>TV No.</th><th>Name / Header</th><th>Department</th><th style={{textAlign:'right'}}>Gross kg</th><th style={{textAlign:'right'}}>Net kg</th><th>Contents</th><th></th></tr></thead>
            <tbody>
              {filtered.map(tv => {
