...
export function TimesheetsPanel({ type }: { type: TsType }) {
  ...
  async function saveWeek(week: WeeklyTimesheet) {
    ...
    try {
      await writeTimesheetCostLines(week, activeProject!.id, rateCards, tceLines, resources)
    } catch { /* non-critical */ }
    toast('Saved', 'success')
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
                    ...
                    <div style={{ display: 'flex', gap: '4px' }} onClick={e => e.stopPropagation()}>
                      {s.status !== 'approved' && <button className="btn btn-sm" style={{color:'var(--green)',fontSize:'10px'}} title="Quick approve" onClick={async()=>{
                        ...
                        try { await writeTimesheetCostLines({...s, status:'approved'}, activeProject!.id, rateCards, tceLines, resources) } catch { /* non-critical */ }
                        load()
                      }}>✓ Approve</button>}
